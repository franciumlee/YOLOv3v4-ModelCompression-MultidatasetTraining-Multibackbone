...
def test(cfg,
    ...
    if model is None:
        ...
        model = Darknet(cfg, imgsz, quantized=quantized, a_bit=a_bit, w_bit=w_bit,
                        FPGA=FPGA, is_gray_scale=is_gray_scale, maxabsscaler=maxabsscaler,alpha_bits = alpha_bits)
        if fuse_bn == True:
            model.fuse()
        # 
        # Load weights
    ...
    if save_json and map and len(jdict):
        ...
        try:
            ...
            cocoGt = COCO(glob.glob('/work/m107061466/cocoapi/annotations/annotations/instances_val*.json')[0])  # initialize COCO ground truth api
...
if __name__ == '__main__':
    ...
    opt.cfg  = opt.cfg  #list(glob.iglob('./**/' + opt.cfg , recursive=True))[0]  # find file
    opt.data = opt.data #list(glob.iglob('./**/' + opt.data, recursive=True))[0]  # find file
